 ...
 */
static int
gitsoo_is_ignored_char(const UTF32Char ustr)
{
    switch (ustr) {
        case ' ': case '\f': case '\n': case '\r': case '\t': case '\v':
 ...
 */
int
token_to_postings_list(gitsoo_env *env,
 ...
 */
int
text_to_postings_lists(gitsoo_env *env,
 ...
 */
void
dump_token(gitsoo_env *env, int token_id)
{
    int token_len;
    const char *token;
