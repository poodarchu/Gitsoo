...
/**
 * 用途：将文档添加到数据库中，建立倒排索引
 * @param[in] env 存储着应用程序运行环境的结构体
 * @param[in] title 文档标题，为NULL时将会清空缓冲区
 * @param[in] body 文档正文
 */
static void
add_document(gitsoo_env *env, const char *title, const char *body)
{
    if (title && body) {
        UTF32Char *body32;
 ...
 */
static void
parse_compress_method(gitsoo_env *env, const char *method,
                      int method_size)
{
