...
#define BUFFER_INIT_MIN 32;

/**
 * 将错误信息输出到标准错误输出
 * format 可以传递给函数printf的格式字符串
 * ... 要传递给格式说明符号（format specifications）的参数
 * return 已输出的字节数
 */
 ...
 * ustr 输入的字符串（UTF-32）
 * ustr_len 输入的字符串的长度。-1表示输入的是以NULL结尾的字符串
 * str 存储转换后的字符串（UTF-8）的缓冲区,该缓冲区要足够大，不得小于ustr_len * MAX_UTF8_SIZE
 * str_size 转换后的字符串的字节数。调用时可将该参数设为NULL
 * return 转换后的UTF-8字符串
 */
 ...
 * str 输入的字符串（UTF-8）
 * str_size 输入的字符串的字节数。-1表示输入的是以NULL结尾的字符串
 * ustr 转换后的字符串（UTF-32）。由调用方释放
 * ustr_len 转换后的字符串的长度。调用时可将该参数设为NULL
 * retval 0 成功
 */
